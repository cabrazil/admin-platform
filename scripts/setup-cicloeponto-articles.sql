-- Script para criar artigos do CicloePonto
-- Execute APÓS criar as categorias E o Author
-- Execute este script no SQL Editor do Supabase
--
-- ORDEM DE EXECUÇÃO:
-- 1. setup-cicloeponto-categories.sql (criar categorias)
-- 2. setup-cicloeponto-author.sql (criar author)
-- 3. setup-cicloeponto-articles.sql (este script - criar artigos)

-- 1. Verificar se as categorias existem
SELECT id, title, slug FROM "Category" WHERE "blogId" = 4 ORDER BY title;

-- 1.1. Verificar se existe pelo menos um Author
SELECT id, name, email FROM "Author" WHERE "blogId" = 4;

-- 2. Criar artigos para CicloePonto (blogId=4)
-- Nota: Substitua os categoryId pelos IDs reais das categorias criadas
...
-- 3. Verificar se os artigos foram criados
SELECT 'Artigos criados para CicloePonto:' as info, COUNT(*) as total FROM "Article" WHERE "blogId" = 4;

-- 4. Listar os artigos criados
